...
| `--only <storypath>`        | Only run a single story or a subset of stories. Use the `title` from the story's default export as the story path, followed by the desired story names. For example, if selecting all stories from a single story file with a `title` of `Example/Button`, this would be `Example/Button/*`. Globs are supported via [picomatch]. This option implies `--preserve-missing`. |
