...
Configure CI to run your visual tests whenever you push code and get pull request badges to get notified about test and review results.
...
Integrate with popular CI tools like you would any other job. Run `npm run chromatic` to start a Chromatic build. If [UI Test](/docs/test) or [UI Review](/docs/review) are enabled, it will return a non-zero exit code when there are changes. For example:
