---
title: Visual tests
description: Chromatic is a cloud-based toolchain that integrates with Storybook, Vitest, Playwright, and Cypress to help teams test and review UI components.
sidebar: { order: 1 }
---
...
### <img src="/docs/assets/vitest.svg" style="align-self: center;" class="inline-icon" alt="" /> Visual testing with Vitest

[Vitest](https://vitest.dev/) is an open source testing framework built on Vite. It allows you to write unit tests to verify functionality, and integration tests to validate how components work together. With [Browser Mode](https://vitest.dev/guide/browser/), Vitest can also run tests in a real browser environment.

Chromatic provides a plugin to integrate with Vitest. While your tests run, Chromatic captures an archive of your component (DOM, styles, and assets). After the tests complete, Chromatic uploads the archives to the cloud, where it renders the components in multiple browsers, captures snapshots, and performs pixel diffing to identify any unintended visual changes.

Vitest offers built-in visual testing capabilities, but the snapshots must be stored in your repository and managing flake and inconsistent test results can be challenging. Chromatic's cloud-based snapshotting and flake-reduction tools make it a better testing solution for teams.

[**Get started with Vitest & Chromatic »**](/docs/vitest)

![](../../images/workflow-vitest-visual-test.png)
