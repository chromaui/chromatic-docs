...
<IntegrationSnippets>
  ...
  <Fragment slot="vitest">
    ```tsx title="tests/UserAccount.test.tsx"
    ...
    describe("UserAccount", () => {
      // Sets the threshold for 0.2 for every test in this suite.
      configure({ diffThreshold: 0.2 });

      test("Renders the user information", async () => {
        await render(<UserAccount />);
        await expect.element(page.getByText("Welcome, username")).toBeInTheDocument();
      });
    });
    ```
  </Fragment>
