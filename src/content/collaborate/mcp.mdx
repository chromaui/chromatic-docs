---
title: MCP
description: Learn deploy a Storybook MCP server using Chromatic.
sidebar: { order: 7 }
---
...
Published MCP server access also enables cloud-based coding agents like Claude Code and GitHub Copilot to access the component metadata and stories.
...
In order to deploy your Storybook MCP server with Chromatic you need enable the MCP feature in your Storybook. If you've already done that, you can skip to step 3 to install the MCP server in your coding agent.
...
### 3. Run a Chromatic build to publish your MCP server
...
### 5. Install MCP server in your coding agent
...
1. Press `CMD`+`SHIFT`+`P` then search for and select the option: `MCP: Add Server…`
...
## Manifest (ignore stuff with tags)
