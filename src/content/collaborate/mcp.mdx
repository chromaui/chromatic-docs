...
You can use [Storybook composition](/docs/composition) to include content from both Storybooks in the local MCP server, allowing your agent to find and reference components from all composed Storybooks when generating UI.
...
Run the following command to configure the local MCP server for your coding agent. We only need to connect local MCP server since the remote Storybook is composed into the local Storybook.
