...
<summary> I hit error 'does not support dynamic client registration' when authenticating </summary>
This error typically occurs when you're trying to connect Chromatic MCP to a tool that does not support
dynamic client registration. The alternative is using the bearer token:
...
1. Find the `token` in the Application tab of the <strong>Developers Tools</strong> under chromatic.com cookies.
   Use the `token`, not the `asset_token`.
2. This method is only recommended for tools that don't support OAuth 2.0 with dynamic client registration. For other tools, always prefer using the `mcp-add` CLI with the `--client-id` parameter, as it handles authentication more securely.
