...
<details>
<summary><b>My custom domain is "Connected", but the page hits error 404. Why?</b></summary>

When you set a custom domain, you select a target branch, and the domain links to the most recent build on that branch. If the domain hits error 404, it's likely because the selected branch doesn't have any Chromatic builds yet.
