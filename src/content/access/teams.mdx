...
3. **Base project role:** the minimum role every member has on every project. See [default roles](#default-roles).
...
You can work from either side. Both routes create the same assignment, so use whichever matches what you're configuring.

**From a team**, when one team needs access to many projects:
...
**From a project**, when one project needs to be shared with many teams:
...
**Base project role** is the minimum role every organization member has on every project. It's required for team-based access to take effect. Options are Viewer, Reviewer, Developer, or None.
