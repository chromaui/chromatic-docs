...
Builds that contain visual changes need to be [verified](/docs#verify-ui-changes). They will fail if you are not using `--exit-zero-on-changes`. Once you accept all the changes, re-run the workflow and the `Chromatic` job will pass.
