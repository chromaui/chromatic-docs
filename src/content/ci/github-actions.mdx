...
All other event types except `pull_request` will not work. However, using the `pull_request` event might cause Chromatic's baselines to be lost in certain scenarios.
