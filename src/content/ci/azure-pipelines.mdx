      ...
      - stage: UI_Tests
        ...
        jobs:
          - job: Cypress
            ...
            container: cypress/browsers:node-22.14.0-chrome-135.0.7049.95-1-ff-137.0.2-edge-135.0.3179.85-1
            variables:
              npm_config_cache: $(Pipeline.Workspace)/.npm
            steps:
              - checkout: self
...
Builds that contain visual changes need to be [verified](/docs#verify-ui-changes). They will fail if you are not using `--exit-zero-on-changes`. Once you accept all the changes, re-run the pipeline and the `Run Chromatic` step will pass.
