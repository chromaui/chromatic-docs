     ...
     pipeline {
       ...
       stages {
         ...
         stage('Cypress') {
           agent {
             docker {
               image 'cypress/browsers:node-22.14.0-chrome-135.0.7049.95-1-ff-137.0.2-edge-135.0.3179.85-1'
               reuseNode true
             }
           }
           environment {
...
[UI Tests](/docs) and [UI Review](/docs/review) rely on [branch and baseline](/docs/branching-and-baselines) detection to keep track of [snapshots](/docs/snapshots). We recommend the following configuration.
...
Builds that contain visual changes need to be [verified](/docs#verify-ui-changes). They will fail if you are not using `--exit-zero-on-changes`. Once you accept all the changes, re-run the build and the `Run Chromatic` pipeline stage will pass.
