...
<Tabs>
  <TabItem label="yarn">
    ```shell
    yarn add --dev chromatic chromatic-playwright @storybook/addon-essentials@next @storybook/server-webpack5@next storybook@next
    ```
  </TabItem>
  <TabItem label="npm">
    ```shell
    npm install --save-dev chromatic chromatic-playwright @storybook/addon-essentials@next @storybook/server-webpack5@next storybook@next
    ```
  </TabItem>
</Tabs>


### 3. Update your Playwright tests with chromatic-playwright imports:
```js
// ➖ Remove this line
...
### 5. Then run a Chromatic build through the CLI (using the token you noted above):

<Tabs>
  <TabItem label="yarn">
    ```shell
    yarn chromatic@latest --build-script-name=build-archive-storybook -t=<TOKEN>
    ```
  </TabItem>
  <TabItem label="npm">
    ```shell
