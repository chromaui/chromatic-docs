...
## Usage with Vitest, Playwright, or Cypress

When pairing Chromatic with Vitest, Playwright, or Cypress, you have to pass the corresponding flag: `--vitest`, `--playwright`, or `--cypress`. Chromatic captures an archive of your UI (DOM, styles, and assets) while Vitest, Playwright, or Cypress runs your tests. The CLI uploads the archive to the Chromatic cloud infrastructure to run visual testing.

```shell
# For Chromatic with Vitest
...
For Vitest, Playwright, and Cypress, we recommend installing the chromatic package as a dependency. This ensures that the CLI is in sync with the `@chromatic-com/vitest`, `@chromatic-com/playwright`, or `@chromatic-com/cypress` package.
