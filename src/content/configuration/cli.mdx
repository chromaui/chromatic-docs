...
The CLI defaults to running with Storybook. It builds and uploads your Storybook to Chromatic's cloud infrastructure, to start the [publish](/docs/storybook/publish) and [visual test](/docs/test) workflows.
...
Note: some options are only available as flags. For a full list of available options, please refer to the [configuration options](/configuration-options) page.
...
Chromatic builds are authenticated using the secret `project-token` for your project. You can access you project token from the Chromatic website during onboarding or from your project's Manage page.

#### Continuous Integration

For setting up CI, store your project token as the `CHROMATIC_PROJECT_TOKEN` environment variable or secret. The CLI automatically picks up the `CHROMATIC_PROJECT_TOKEN` environment variable so you won't have to explicitly provide it.
