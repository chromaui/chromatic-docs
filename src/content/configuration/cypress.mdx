...
Run the following command to install chromatic-cypress and related dependencies.

<Tabs>
  <TabItem label="yarn">
    ```shell
    yarn add --dev chromatic chromatic-cypress @storybook/addon-essentials@next @storybook/server-webpack5@next storybook@next
    ```
  </TabItem>

  <TabItem label="npm">
    ```shell
    npm install --save-dev chromatic chromatic-cypress @storybook/addon-essentials@next @storybook/server-webpack5@next storybook@next
    ```
  </TabItem>
</Tabs>
...
### 5. Set Chromatic archive location

Prepend the build scripts with the Cypress-managed downloads folder
