...
<YouTubeCallout client:load id="XF4IZlIZGz8" summary="Mocking network requests with MSW addon" />
...
<YouTubeCallout client:load id="TL-YSDsPpCk" summary="Module mocking in Storybook" />

## Vitest

You can [mock network requests in Vitest](https://vitest.dev/guide/mocking/requests.html) using Mock Service Worker (MSW).

<Tabs>
  <TabItem label="Setup">
<br />
MSW recommends integrating it with Vitest by [extending the test context](https://vitest.dev/guide/test-context.html#extend-test-context).

```ts title="test-extend.ts"
...
```
  ...
  <TabItem label="In a test">
<br />
Now you can use the `worker` object in your tests to mock network requests. You can add request handlers to the worker for specific test cases, and reset them after each test to avoid side effects.

```jsx title="Dashboard.test.tsx"
import { http, HttpResponse } from 'msw';
import { test } from './test-extend';
import { Dashboard } from './components/dashboard';

test('displays a notification if fetching the dashboard failed', async ({ worker }) => {
...
Vitest also allows you to mock modules using the powerful [`vi.mock()` function](https://vitest.dev/guide/mocking/modules.html). This is useful when you need to mock third-party libraries that make network requests, or when you want to isolate your tests from external dependencies.
