...
If you don't have Storybook for React Native set up yet, the quickest option is to use the official setup skill, which scaffolds everything for you:

```shell
npx skills add https://github.com/storybookjs/react-native --skill setup-react-native-storybook
```

Then run `/setup-react-native-storybook` in your coding agent.
...
const StorybookUIRoot = view.getStorybookUI({
  ...
  port: parseInt(
    process.env.EXPO_PUBLIC_STORYBOOK_WEBSOCKET_PORT || "7007",
    10,
  ),
...
If you have a custom build process, you can also build the artifacts yourself and pass the directory to `chromatic` with the `--output-dir` flag. More on that in the [advanced configuration](#reusing-an-existing-build) docs.
...
Pass `--storybook-build-dir` to skip all build steps except `manifest.json` generation. This is most useful for parallelizing native builds across CI machines: one machine builds the Android artifact, another builds iOS, and a third runs Chromatic with `--storybook-build-dir` pointing to the directory containing both artifacts.

### The `react-native-build` command

The CLI includes a `react-native-build` sub-command for building your React Native app. It uses the same logic as a regular Chromatic run, but lets you split your CI pipeline and parallelize Android and iOS builds.
...
Here's a sample GitHub Actions workflow that builds the Storybook app for iOS and Android in parallel, then runs Chromatic with the generated artifacts:
