...
Components sometimes trigger custom interactions when they render, such as [JavaScript-driven animations](/docs/animations#javascript-animations) or third-party functionality outside of your control. The `delay` configuration option defines a fixed minimum wait before Chromatic captures a snapshot, allowing the test to reach its intended state.
...
<IntegrationSnippets>
  ...
  <Fragment slot="vitest">
    ```ts title="tests/Categories.spec.tsx"
    ...
    describe("Categories Page", () => {
      // Configures the delay (in milliseconds) for this test suite
      configure({ delay: 300 });

      test("Renders the categories page", async () => {
        await render(<Categories />);
        await expect.element(page.getByText("Available Categories")).toContain("Available Categories");
      });
    });
    ```
  <ParamsCallout name="delay" integration="vitest" />
  </Fragment>
