...
UI components respond and adapt to different device widths. Chromatic streamlines visual testing, enabling you to capture each test at multiple viewport sizes. Viewports is compatible with Storybook, Vitest, Playwright, and Cypress integrations.
...
Viewports in Vitest can be [configured globally](https://vitest.dev/guide/browser/#configuration) in your main Vitest configuration file as follows:
...
test('Page display on medium size screen', async () => {
  // 👇 Overrides the option in the test.
  await page.viewport(1600, 1200);
