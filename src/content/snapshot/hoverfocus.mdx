...
Components can respond differently to hover or focus events. Here are a few techniques for capturing the result of these user events in Chromatic.

## JavaScript-triggered hover states

If you're working with a component that relies on JavaScript to trigger hover states (e.g., tooltips, dropdowns), you can adjust your tests and include Storybook's [play](https://storybook.js.org/docs/writing-stories/play-function) function or [Vitest's](https://vitest.dev/guide/browser/interactivity-api#userevent-hover), [Playwright's](https://playwright.dev/docs/api/class-locator#locator-hover), and [Cypress's](https://docs.cypress.io/api/commands/hover) APIs to simulate the state and verify the component's behavior.
...
<IntegrationSnippets>
  ...
  <Fragment slot="vitest">
    ...
    test("Warns the user about invalid credentials", async () => {
      await render(<LoginForm />);

      await page.getByLabelText("email").fill("test@email.com");
      await page.getByLabelText("password").fill("password");

      // Triggers the hover state
      await page.getByLabelText("password").hover();

      await expect
        .element(page.getByText("Must be at least 16 characters long"))
        .toBeVisible();
    });
    ```
  </Fragment>
...
<div class="aside">
  ℹ️ This approach requires manually toggling the class names in your component's test to simulate
  the necessary states. If you're using a CSS-in-JS framework, you can
  [automate](https://github.com/Workday/canvas-kit/pull/377/files) this process by creating a
  JavaScript wrapper that adds the class names programmatically.
</div>
