...
<IntegrationSnippets>
  ...
  <Fragment slot="vitest">
    ...
    describe("Authentication", () => {
      // Enables the forcedColors option for every test in this suite.
      configure({ forcedColors: "active" });

      test("Authenticates a user with high contrast mode enabled", async () => {
        await render(<LoginForm />);

        await page.getByLabelText("email").fill("test@email.com");
        await page.getByLabelText("password").fill("KC@2N6^?vsV+)w1t");
        await page.getByRole("button", { name: "Login" }).click();
      });
    });
    ```
    <ParamsCallout name="forcedColors" integration="vitest" />
  </Fragment>
...
<IntegrationSnippets>
  ...
  <Fragment slot="vitest">
    ...
    describe("Authentication", () => {
      // Enables the prefersReducedMotion option for every test in this suite.
      configure({ prefersReducedMotion: "reduce" });

      test("Authenticates a user with reduced motion enabled", async () => {
        await render(<LoginForm />);
    ...
    ```
    <ParamsCallout name="prefersReducedMotion" integration="vitest" />
  </Fragment>
  <Fragment slot="playwright">
    ```ts title="tests/Auth.spec.js|ts"
    import { test } from "@chromatic-com/playwright";
