...
Chromatic proactively pauses CSS transitions, CSS and SVG animations, and videos to prevent false positives. Variables outside of Chromatic's control can otherwise make animation timing unstable from one test run to the next.
...
<IntegrationSnippets>
  ...
  <Fragment slot="vitest">
    ```tsx title="tests/Product.test.tsx"
    ...
    describe("Product", () => {
      // Overrides the default behavior and pauses the animation at the first frame for every test in this suite.
      configure({ pauseAnimationAtEnd: false });

      test("Renders the product details", async () => {
        await render(<Product />);
        await expect.element(page.getByText("Product details")).toBeInTheDocument();
      });
    });
    ```
  <ParamsCallout name="pauseAnimationAtEnd" integration="vitest" />
  </Fragment>
...
If you're working with JavaScript animation libraries (e.g., [Motion](https://motion.dev/)), Chromatic doesn't have the same control over them as it does over CSS animations and won't disable them by default. We recommend disabling JavaScript animations when running in Chromatic to keep tests stable.
...
Chromatic automatically pauses videos and animated GIFs at their first frame, keeping visual tests stable without custom workarounds. If you specify a [poster](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/video#poster) attribute for videos, Chromatic will use that image instead.
