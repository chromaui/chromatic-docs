...
Chromatic detects UI changes but it’s still up to you to verify if changes are intentional. For intentional changes, you need to update the baseline so future tests will be compared to the _latest baseline_ for the page. If a change is unintentional it needs to be fixed.

- ✅&nbsp;**Accept change**: This updates the page baseline. When a snapshot is accepted it won’t need to be re-accepted until it changes, even through git branches or merges.
...
Sometimes you need a closer look to determine why a snapshot is rendering as it does. Along with pixel and DOM diffs, Chromatic displays the interactive UI just as it appears in your app and E2E tests.
...
If you accept all the changes, the build will **🟢&nbsp;Pass**. Future builds whose pages have the same appearance will pass.
...
After you merge your code, Chromatic will also apply accepted baselines to pages on the target branch. That means you’ll only need to accept baselines a single time.
