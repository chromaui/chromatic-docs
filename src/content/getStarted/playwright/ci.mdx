...
description: Learn how to configure Chromatic for Playwright with your continuous integration service
sidebar: { order: 3 }
---

import TroubleshootingCI from "../../shared-snippets/ci/troubleshooting.md";

# Automate Playwright visual tests with CI
...
Integrate with popular CI tools like you would any other job. Run `npm run chromatic` to publish your Storybook. If [UI Test](/docs/playwright/test) or [UI Review](/docs/playwright/review) are enabled, it will return a non-zero exit code when there are changes. For example:

```yml
- run:
    command: npm install # install dependencies
- run:
    command: npm run playwright-test # run your playwright tests
- run:
...
Chromatic provides a [GitHub Action](https://github.com/marketplace/actions/publish-to-chromatic) to help you automate your Playwright visual tests. Use this Action with the `playwright` flag to run a build.
...
Now that you've set up Chromatic, learn the recommended workflow in development and during the pull request. Or, explore the advanced configuration options to tailor your Chromatic and Cypress setup to your needs.

<a class="btn primary round" href="/docs/in-development">
  Learn workflow
</a>

<a class="btn outline round" href="/docs/playwright/configure">
