...
Integrate with popular CI tools like you would any other job. Run `npm run chromatic` to publish your Storybook. If [UI Test](/docs/cypress/test) or [UI Review](/docs/cypress/review) are enabled, it will return a non-zero exit code when there are changes. For example:
...
Chromatic provides a [GitHub Action](https://github.com/marketplace/actions/publish-to-chromatic) to help you automate your Cypress visual tests. Use this Action with the `playwright` flag to run a build.
...
<a class="btn outline round" href="/docs/cypress/configure">
