...
Snapshot Billing Rate is billed snapshots divided by total snapshots. It shows whether your usage is becoming more or less efficient over time.

When you first set up Chromatic, the rate starts at `1.0`. Chromatic captures a new snapshot for every test and bills it at full cost.

We invented [TurboSnap](/docs/turbosnap) to use changed files to determine which stories need new snapshots. It _copies_ or _bypasses_ snapshots for stories it determines could not have changed. Both are billed at a lower cost, lowering the average Snapshot Billing Rate.
...
| Pattern                                          | What it suggests                                         | What to check                                                                                                            |
| ------------------------------------------------ | -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| Snapshot Billing Rate increased                  | A larger share of snapshots was billed at full cost      | Use [TurboSnap Effectiveness Breakdown](#why-did-the-rate-increase) to see why more snapshots were captured at full cost |
