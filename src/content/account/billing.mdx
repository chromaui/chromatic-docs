...
| `.chromatic-ignore`, `data-chromatic="ignore"`, or `ignoreSelectors` parameter | Chromatic excludes a specific area or element from visual comparisons. The visual snapshot is still captured and billed.                     |
| TurboSnap                                                                      | The billed snapshot cost is 0.2 per copied snapshot. Bypassed builds generate 0 billed snapshots.                                            |
| `onlyStoryNames` / `onlyStoryFiles`                                            | Stories excluded by these options are billed at the copied snapshot rate of 0.2 billed snapshots.                                            |
| `--skip` flag                                                                  | The build is skipped entirely and generates 0 billed snapshots.                                                                              |
| Unstable, ignored, or quarantined stories                                      | Stories are still captured, even if their diffs are not reviewed. Each story captured generates 1 billed snapshot.                           |
