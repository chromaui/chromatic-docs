...
Chromatic bills customers monthly based on their subscription date. For instance, if you subscribed on February 14th, you'll be billed on the 14th of each month. Any additional snapshots used during that time are added to the current billing period. Similarly, snapshots are allocated within the same time frame and reset following the same cadence.
...
Running an [accessibility test](/docs/accessibility) counts as one additional snapshot per test. However, you are not charged for [interaction](/docs/interactions) tests.
...
Each plan comes with a specific number of included snapshots per month. If your usage reaches that amount:

1. **Free plan:** Review and testing will be paused once you use all 5,000 included free snapshots per month. You can resume testing by upgrading to a paid plan or waiting until your included snapshots reset at the end of the billing cycle. ![Builds blocked because the account hit the monthly snapshot limit](../../images/build-blocked.png)
...
For information on the number of snapshots included in each plan and the cost of additional snapshots, please refer to the [pricing page](https://www.chromatic.com/pricing). You can also set [usage alerts](/docs/billing#usage-alerts) to notify a designated email address when your usage reaches a certain number of snapshots.
...
The **invoice** includes details of the active plan for the billing period, the base number of snapshots included, charges for additional snapshots, and a link to download a PDF copy of the invoice (available for all billing periods).
