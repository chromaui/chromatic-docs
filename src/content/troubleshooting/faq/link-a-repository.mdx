...
### Can I limit GitHub OAuth to selected repositories?

No. GitHub OAuth App approval cannot be limited to selected repositories. The picker shows repositories the signed-in GitHub profile can access. Restricting `chromatic-com` to selected repositories does not change that list. Revoking OAuth removes the organization from add and link.

Use GitHub App selected-repository install to limit pull request checks and UI Review, not to hide repositories from the picker. [Why do extra repositories appear after I limit the GitHub App? »](/docs/faq/org-not-appearing#why-extra-repositories-appear-after-i-restrict-the-github-app)
...
Add or link can also fail with **Could not retrieve repository ID** when `chromatic-com` is installed on selected repositories and that repository is not on the list. Grant the GitHub App access to the repository, then try again. [Could not retrieve repository ID when adding a project »](/docs/faq/org-not-appearing#could-not-retrieve-repository-id-when-adding-a-project)
