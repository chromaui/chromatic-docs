...
| `idleNetworkInterval`    | `number`                          | Time in milliseconds to determine whether network is idle. The network is considered idle if there are no new network requests for at least this duration. **Only available as global option.**                                  |
| `outputDirectory`        | `string`                          | Directory where temporary archives and snapshots will be stored. Relative to the project [the project `root`](https://vitest.dev/config/root.html#root). Defaults to `.vitest/chromatic`. **Only available as a global option.** |
...
| `reporter`               | `boolean \| { verbose: boolean }` | Test reporter used to log information about captured archives. Verbose mode controls whether status is logged during test run. A summary is always logged at the end of test run. **Only available as a global option.**         |
