...
sidebar: { order: 5, label: "Trace Utility" }
...
For comparison, you can run the same command for an imported styles file and see if there's any difference:
...
1. **Ensure you've built your Storybook to write your stats JSON**
...
Change the verbosity of the output using the `-m`/`--mode` option to `expanded` to see underlying modules:
...
Changing the `--mode` to `compact` will print only the list of dependent story files without showing how they're connected:
...
By default, the `trace` utility expects your stats file to be in the `storybook-static` directory as either `preview-stats.json` or `preview-stats.trimmed.json`. If you're using a custom build directory, your stats file is output to a different directory than your build dir, or it's under a different name, you can pass the `-s`/`--stats-file` option to specify the custom stats file name:
...
If you're running the `trace` utility out of a monorepo with multiple Storybook projects or your project is nested, you can pass the `-b`/`--base-dir` option to specify the package directory:
...
If the second command shows fewer affected stories, you've identified the barrel file as the culprit. Barrel files commonly increase the blast radius of your changes since they may introduce unrelated files to a components dependency graph. The solution is to skip the barrel file and use direct imports:
...
Direct imports ensure the dependency graph can follow the file without tracing unrelated components. This is most critical in global configurations (such as your preview file) and barrel files with a large impact radius.
...
A uchange to this tility function affects multiple components unexpectedly. The output shows it affects:
...
It's necessary to review how the currency utility is being imported by the Admin Dashboard component and other components being imported into Admin Dashboard. If you discover a third file cascading the changes to `AdminDashboard.jsx`, you may test ignoring it:
...
Utilize these git commants along with `trace` to confirm what files have changed and the impact based on your dependency graph:
...
If dependencies are genuinely used across many components, document them to create institutional knowledge so your team can assess ripple effects before making modifications. When dependencies are part of a shared library or design system that are being tested separately (or can be isolated for better scoping), [importing those components from the built package](/docs/turbosnap-for-monorepos/#prefer-importing-from-built-packages-over-src) can help avoid unnecessary changes being detected since built packages are generally excluded from git.
