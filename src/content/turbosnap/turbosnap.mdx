...
3. Using Webpack's dependency graph, TurboSnap identifies the individual story files affected by those git-identified changes.
...
1. Check your `preview.js` file: you may have a global decorator configured that's [imported from an index or barrel file](/docs/setup-turbosnap/#why-changes-are-not-being-detected-correctly), which itself imports a bunch of other files.  This can lead to *all* stories depending on a large swath of seemingly unrelated files.<br /><br />
Whenever a global Storybook file changes, we run a complete build because global Storybook files can change how *any* story renders.  If any of those files or their imports change, we will trace them as dependent changes.  If the file changes are frequent, you can utilize `--untraced` on these files to help ensure you're not triggering a complete rebuild each time the file changes.

<div class="aside">

Learn more about [how to avoid re-testing for certain files](/docs/setup-turbosnap/#avoid-re-testing-dependent-stories-when-certain-files-changed).

</div>

2. Make sure you have a valid lockfile(s) available and that it's in sync with your `package.json`.  If a lockfile is missing or out ouf sync with `package.json`, we'll have to re-test all stories.

<div class="aside">

Learn more about how to [avoid re-testing on changes to package control files](/docs/setup-turbosnap/#avoid-re-testing-on-changes-to-package-control-files).
...
When you have a merge commit, we look at both ancestors tied to a build and pull any changes from either build. Chromatic considers **any file that has changed since either ancestor’s commit** to decide if a story needs to be re-snapshotted. In other words, we take the *union* of the git changes and not an intersection of them.
...
Since most folks want to preserve their branch's baseline history, we do this as the default and have a flag available for anyone who wants to ignore the baselines for the last build on a branch (`ignoreLastBuildOnBranch`).  Usually, you would only use this option in rare cases where you were doing the rebase in order to *delete* the baseline changes for the branch.
