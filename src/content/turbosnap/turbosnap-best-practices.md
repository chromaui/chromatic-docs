...
Most everyone agrees it's important to test your UI, but it's even more important to make sure you're testing the _right parts_ of your UI at the _right time_. Meet TurboSnap: one of Chromatic's most powerful tools for accelerating UI testing in CI. By using your git history and dependency graph to intelligently detect what's changed in your Storybook project, Chromatic only tests components affected by your updates, saving time and CI resources while maintaining the value of your tests.

Whether you're working in a single-project repository or a more complex setup (such as a monorepo), following best practices ensures TurboSnap works efficiently and surfaces _meaningful_ visual regressions. Here are some essential tips to keep your TurboSnap running fast, reliably, and accurately.

## Confirm TurboSnap is enabled

Once you've met the [prerequisites](/docs/turbosnap/setup#prerequisites) and started using TurboSnap, it should appear as enabled on the "Manage" page of your project. TurboSnap is enabled through the `onlyChanged` option. You can make sure it's enabled for your builds by verifying it's listed within your Chromatic script:
...
These two options are resolved on different bases. See [TurboSnap config paths](/docs/turbosnap/config-paths) for which options are relative to your repository root and which are relative to your working directory.
...
Run `npx @chromatic-com/turbosnap-helper` from the root of your repo to have the helper provide you with the accurate directories and even automatically update your config file—no more guesswork!

</div>

## Run with caution when using the `pull_request` event

TurboSnap is not compatible with the `pull_request` event trigger. This is because the workflow runs against an ephemeral merge commit, which can result in issues tracking baselines and locating earlier builds.
...
Setting `ref` is one of the most critical parts to using the `pull_request` event, as the `ref` parameter defines what gets checked out into the workspace, which in turn impacts how Chromatic analyzes your build. By setting `ref` to `${{ github.event.pull_request.head.ref }}` ensures that GitHub checks out the correct branch and that Chromatic automatically detects accurate git information.
...
- name: Publish to Chromatic
  ...
  env:
    ...
    # 👇 Makes sure this is always set to the correct owner/repo
    CHROMATIC_SLUG: ${{ github.repository }}
```

<div class="aside">

These values match what you'd want TurboSnap and Chromatic to see, but _only if the correct code is checked out_. If you're not using `ref` to ensure the correct commit is checked out, setting your git environment variables _will not help_. Without `ref`, your env variables could describe one commit, while the git workspace contains another, causing TurboSnap to have unpredictable results.
...
TurboSnap relies on lockfiles to obtain actual version numbers, rather than semver ranges, to determine an exact set of changed dependencies for more accurate tracing. If your lockfile is missing or out of sync with `package.json`, TurboSnap can't compute the changes, resulting in a full rebuild.
...
Your Storybook preview file (`.storybook/preview.js|ts`), where you define global decorators, parameters, and theming, is crucial for TurboSnap rebuilds. Any change to this file or its imports can affect how stories are rendered in your project, even if only a single component is affected. This means changes to theme object definitions, global decorators, context providers, mock setups, and global parameters will trigger a full rebuild.
...
TurboSnap depends on the files your bundler processes. Therefore, it's crucial that files not processed by your bundler also trigger tests. This helps prevent accidental regressions that could have been caught during testing.

The `externals` feature was designed specifically for this purpose. It directs TurboSnap to monitor specified files or glob patterns for changes, triggering a rebuild whenever any of them are modified.
...
If a file or folder is frequently updated and has a broad but legitimate impact on your stories, consider scoping it into a dedicated package or a separate Storybook project to reduce its impact radius. This allows TurboSnap to trace only stories affected by the package, which can be tested in isolation using Storybook and Chromatic. Here are a few basic guidelines to help you decide when to split:

- Theme file changing daily? Move it to its own package and project.
...
- Preview file importing volatile code? Move it out of preview and apply it in stories or wrappers to keep the preview stable.
...
The best practices outlined above will help you achieve consistent, fast builds with minimal overhead, without sacrificing test coverage.

In summary, limit changes to necessary updates only. Modularize global code, declare externals for non-bundled assets, and audit your `preview-stats.json` file using the [trace utility](/docs/turbosnap/troubleshooting#why-are-no-changes-being-detected).
