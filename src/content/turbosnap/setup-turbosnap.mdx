...
<div class="aside">
  ⚠️ When using TurboSnap, your builds may complete in less time using fewer snapshots. However, we
  don't allow using TurboSnap immediately when starting out with Chromatic since the configuration
  is more complicated and can lead to difficult-to-debug scenarios or UI changes being missed.
  Instead, become familiar with Chromatic's out-of-the-box behavior, and once your project has been
  running smoothly, consider trying out TurboSnap. TurboSnap is unlocked after ten successful builds
  on CI.
</div>
...
<div class="callout">
  ℹ️ Most of this guide is for TurboSnap with Storybook. To set up TurboSnap with Vitest, follow the
  [dedicated setup guide](/docs/vitest/turbosnap).
</div>
...
- You have other files outside the Webpack dependency tree that affect your stories (e.g., Sass or template files)
...
TurboSnap Helper supports monorepos. Starting from the root, it can scan all your projects, so you're not accidentally missing assets or settings that could trigger full rebuilds. If you run the utility from your project root, not a problem. The utility will still work just fine.
...
<div class="aside">
  This message indicates how many git changes Chromatic has detected since the last Chromatic build.
  Usually, that's just one or two commits' worth of files.
</div>
...
This message shows the number of story files that depend on the X changes listed above. This message might also be replaced with a message telling you that we need to capture all stories ([see below](/docs/turbosnap#full-rebuilds)).
...
<div class="aside">
  This message tells you how many snapshots we actually took instead of the number of stories we
  found in your Storybook. Usually, C is the number of stories in the Y-component files above.
</div>
...
When TurboSnap is enabled and the `preview-stats.json` is missing, Chromatic doesn't fail the build. Instead, it issues a warning that TurboSnap has **bailed** and runs an unoptimized build.
...
### Avoid re-testing dependent stories when certain files have changed

You may have certain files in your Webpack dependency graph that are (indirectly) used by a story but which you know are unlikely to cause a meaningful (visual) change. A typical example is a global decorator that imports some utility files. Since a global decorator applies to all stories, changing this utility file would require retesting the entire Storybook. You can avoid that problem using the `--untraced` flag:

```shell
chromatic --only-changed --untraced ".storybook/decorators/*.js"
```

TurboSnap works by taking a list of changed files in your Git repository and tracing those down to a set of story files. The `--untraced` flag allows you to skip tracing dependencies for certain files. That means any file in the Webpack dependency graph matching `--untraced` will be ignored, and thus stories (indirectly) depending on it will not get marked for re-testing at that time. However, those stories might still be marked as a result of tracing another changed file (via a different dependency chain).
...
`--untraced` is particularly useful when you're importing "index" files that re-export many underlying modules. A change to any of these modules would mark any file that imports the index file as "dirty", even if it doesn't actually use the changed module. By using `--untraced` on the index file, all its re-exported modules are automatically untraced as well, provided they aren't imported directly.

#### Avoid re-testing on changes to package control files

When certain files related to `node_modules` (`package.json`, `package-lock.json`, `yarn.lock`) change, TurboSnap attempts to determine an exact set of changed dependencies and trace them to their associated stories. We rely on lockfile(s) to obtain actual version numbers rather than semver ranges. TurboSnap retrieves versions for both the current state of the repository and for each baseline commit. If a lockfile is missing or out of sync with package.json, TurboSnap cannot do this, and we'll have to re-test all stories.

Similar to source code changes, the `--untraced` flag can also be used to ignore dependency updates (e.g., `--untraced "services/backend/package.json"`). That way, any dependency updates in that package will not be taken into account when applying TurboSnap. A typical use case scenario would be to untrace the `services/backend/**/*` directory and ignore _any_ changes, including dependencies. If you need fine-grained control over what is untraced, you can also enable it for a specific lockfile, with the caveat that untracing the root-level lockfile will ignore any dependency changes in _all_ packages that rely on it (i.e., sub-packages that don't have their lockfile).
...
<YouTubeCallout client:load id="pmVx8Meh4g0" summary="Configure Turbosnap in a Monorepo" />

When configuring TurboSnap in a monorepo, it's important to ensure that Chromatic understands where your Storybook project is relative to where you're executing your `chromatic` script. This is particularly important because TurboSnap relies on the correct file path resolution to determine which stories have changed and which stories can be skipped. If you're running a subproject in a monorepo, chances are you're executing your script from the root of your repository, in which case, there are extra considerations when configuring TurboSnap.
...
This would ensure we're checking `packages/webapp` for your Storybook project rather than your root directory, which could otherwise detect unexpected package control files from other subprojects. Even though the default config directory is being used, you'll want to ensure to set `--storybook-config-dir` as well, since this path is relative to your current working directory.
...
Using the path from the project's root (`.storybook/decorators/*.js`) would still cause the files to be traced by TurboSnap. Keep in mind you can use a pattern such as `--untraced "**/.storybook/decorators/*.js"`. This would work with TurboSnap and result in changes to the directory being untraced.
...
TurboSnap is _not_ compatible with the `pull_request` trigger or its variations. The reason is that `pull_request` workflows run against an ephemeral merge commit that doesn't yet exist in your git history, but _would_ if you were to merge the PR at that point.
