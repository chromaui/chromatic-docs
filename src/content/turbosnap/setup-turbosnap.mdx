...
- [UI Tests](/docs/storybook/test/#enable) should be enabled
...
Teams frequently use an index file ([barrel file](https://basarat.gitbook.io/typescript/main-1/barrel)) to simplify imports. This practice allows for easier importing, such as accessing a component from `@component-library` instead of having to specify the full path like `@component-library/elements/table`. However, barrel impact TurboSnap's ability to detect changes.
