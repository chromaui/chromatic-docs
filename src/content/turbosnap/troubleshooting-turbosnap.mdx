...
- **Increase Node.js memory allocation:** Try rerunning Chromatic’s CLI with a higher memory limit by setting `NODE_OPTIONS=--max_old_space_size=4096` (4GB) or higher. This provides Node.js with more memory, which can help with larger builds.
...
The reason for this behavior is how Chromatic handles it when it chooses not to re-snapshot a story. In such a case, it "copies" the snapshot for the story from the ancestor build, knowing (due to the git check) that the story cannot have changed in the meantime.

In the case of merge commits, Chromatic does not know ahead of time which side of the merge the snapshot might be copied from, because that involves running the [complete baseline selection](/docs/branching-and-baselines#calculate-a-snapshot-baseline-from-the-ancestor-builds) process. As a result, it needs to be conservative and allow for changes on either branch.
...
| `unavailable`                                                                           | TurboSnap isn't available for your account yet. It becomes available after 10 CI builds.        |
| Empty                                                                                   | Either TurboSnap ran and copied what it could, or it wasn't enabled for that build.                  |

An empty column doesn't distinguish those last two. To tell them apart, open the build in Chromatic and check the TurboSnap status in the test summary. Its tooltip says whether TurboSnap was applied, bailed, or was disabled for that build.

## What do the TurboSnap bail reasons in my usage report mean?

Your [usage data (CSV report)](/docs/billing/usage-reports#export-usage-data-as-a-csv) can help you identify opportunities to improve your TurboSnap configuration. The **TurboSnap Bail Reason** column tells you what caused TurboSnap to bail and trigger a full rebuild. The **TurboSnap effectiveness** chart in the [Trends dashboard](/docs/optimize-usage) segments by these same reasons.
...
You may have made changes to one of the files in your Storybook configuration, or to a dependency imported by your configuration. It's important to keep in mind the second part, since it's common practice to import decorators into the `preview.js` config.
...
TurboSnap bailed due to a missing stats file.

This is due to Chromatic CLI versions before 10.6.0. Newer versions fail the build with an error message instead of bailing, so it only appears in usage reports covering builds that ran on an older CLI.
...
TurboSnap bailed because it couldn't find an ancestor build for the commit.
...
TurboSnap bailed because the commit is a rerun of a previous build, since the baseline build had the same commit and branch name.
...
TurboSnap becomes available once a project has run at least 10 builds from CI, and it remains available for the entire account thereafter. Until that happens, builds that asks for TurboSnap record this reason, and capture every test.
