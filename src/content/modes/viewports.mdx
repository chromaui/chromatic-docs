...
<div class="aside">
  ⚠️&nbsp;&nbsp;While the viewport feature allows you to specify dimensions using any valid CSS unit
  (such as px, rem, calc, etc.), Chromatic modes only support whole numbers or strings with a "px"
  suffix.
</div>
...
Safari and Firefox can't capture pages taller or wider than 32,767 pixels due to rendering engine limitations. This limit is measured in image pixels, so a snapshot captured at a 2.0 [device pixel ratio](/docs/snapshots#device-pixel-ratio) reaches it at half the height in CSS pixels. When that happens, Chromatic automatically retries the capture at DPR 1.0.
...
When you add a viewport, Chromatic will size the browser’s viewport to the defined width and height. It will then take a snapshot and crop it to the component's bounding box. This eliminates negative space around snapshots, reducing the visual information you must review.
